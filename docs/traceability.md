...
| --- | --- | --- | --- | --- | --- |
| Spanish portfolio | React SPA plus manual Express HTML | Astro SSR `/es/` | Build and server-rendered contract | Ready for host validation | PostgreSQL fallback retained |
| English portfolio | React SPA plus manual Express HTML | Astro SSR `/en/` | Build and server-rendered contract | Ready for host validation | PostgreSQL fallback retained |
| SEO metadata | Client effects and duplicated template | Astro layout | Server-rendered contract | Ready for host validation | Canonical, hreflang, OG, Twitter, JSON-LD |
...
| Deployment | Docker Compose and Traefik | Same topology with Astro build and separate PostgreSQL volume | Compose syntax, build, and Express startup-contract tests | In progress | Container runtime and VPS validation pending |
